...
# Official FAA links – prefer disposition tables and CACI worksheets
CACI_MAIN = "https://www.faa.gov/ame_guide/certification_ws"
AME_GUIDE = "https://www.faa.gov/ame_guide"
DISP_INDEX = "https://www.faa.gov/ame_guide/dec_cons/disp"

# Disposition tables / condition pages
ASTHMA_DISP = "https://www.faa.gov/ame_guide/app_process/exam_tech/item35/amd/asthma"
ASTHMA_DISP_PDF = "https://www.faa.gov/ame_guide/media/Asthma_dispo_table.pdf"
HYPERTENSION_DISP = "https://www.faa.gov/ame_guide/app_process/exam_tech/item36/amd/hypertension"
BP_DISP = "https://www.faa.gov/ame_guide/app_process/exam_tech/item55/amd"
GLAUCOMA_DISP = "https://www.faa.gov/ame_guide/app_process/exam_tech/item32/amd"
NEURO_DISP = "https://www.faa.gov/ame_guide/app_process/exam_tech/item46/amd"
PSYCH_DISP = "https://www.faa.gov/ame_guide/app_process/exam_tech/item47/amd/table"
AASI_LIST = "https://www.faa.gov/ame_guide/special_iss/all_classes"
...
with st.sidebar:
    ...
    """)
    st.divider()
    st.header("Disposition Tables & CACI Worksheets")
    st.markdown(f"""
    **Indexes**
    - [All Disposition Tables]({DISP_INDEX})
    - [All CACI Worksheets]({CACI_MAIN})
    - [AME Guide home]({AME_GUIDE})
    - [AASI / Special Issuance list]({AASI_LIST})

    **Disposition tables (condition pages)**
    - [Asthma (Item 35)]({ASTHMA_DISP}) · [PDF]({ASTHMA_DISP_PDF})
    - [Hypertension (Item 36)]({HYPERTENSION_DISP})
    - [Blood Pressure (Item 55)]({BP_DISP})
    - [Ophthalmoscopic / Glaucoma (Item 32)]({GLAUCOMA_DISP})
    - [Neurologic (Item 46)]({NEURO_DISP})
    - [Psychiatric (Item 47)]({PSYCH_DISP})
    ...
    **Other CACIs** → open [All CACI Worksheets]({CACI_MAIN}) and select the condition for the current PDF.
    """)
    st.divider()
...
def show_result(decision, message, item60="", reference=""):
    ...
    if reference:
        ...
        st.markdown(f"**Verify on AME Guide:** [Disposition Tables index]({DISP_INDEX}) | [All CACI Worksheets]({CACI_MAIN}) | [AME Guide home]({AME_GUIDE}) — open the specific disposition table or CACI worksheet for this condition before final action.")
    ...
    if headache_type.startswith("Simple"):
        ...
        if mild == "Yes":
            show_result("Issue",
                ...
                reference=f"[CACI Migraine Worksheet]({MIGRAINE_CACI}) | [Neurologic Disposition (Item 46)]({NEURO_DISP})")
        ...
        if all(x == "Yes" for x in [c1,c2,c3,c4,c5,c6,c7,c8]):
            show_result("CACI Issue",
                ...
                reference=f"[CACI Migraine Worksheet]({MIGRAINE_CACI}) | [Neurologic Disposition (Item 46)]({NEURO_DISP}) | [All CACI]({CACI_MAIN})")
...
if "18.b" in selected:
    ...
    if "Single" in diz:
        show_result("Issue", "Single explained episode with full recovery may be issuable with good documentation.",
                    ...
                    reference=f"[Neurologic Disposition (Item 46)]({NEURO_DISP})")
...
if "18.d" in selected:
    ...
    if "Simple refractive" in eye:
        show_result("Issue", "Meets visual standards with or without correction. Add corrective lens limitation if required.",
                    ...
                    reference=f"[Item 32 Ophthalmoscopic Disposition]({GLAUCOMA_DISP}) | [AME Guide Eyes section]({AME_GUIDE})")
        ...
        if all(x == "Yes" for x in [g1,g2,g3,g4,g5,g6,g7]):
            show_result("CACI Issue", "All CACI Glaucoma criteria appear met.",
                        ...
                        reference=f"[CACI Glaucoma Worksheet]({GLAUCOMA_CACI}) | [Item 32 Ophthalmoscopic Disposition]({GLAUCOMA_DISP})")
        ...
        show_result("Defer", "Significant eye pathology generally requires FAA decision. Obtain Form 8500-7 or specialist report.",
                    reference=f"[Item 32 Ophthalmoscopic Disposition]({GLAUCOMA_DISP})")
    ...
    if ast.startswith("A."):
        ...
        if interfere.startswith("Yes"):
            show_result("Issue", 
                ...
                reference=f"[Asthma Disposition Table]({ASTHMA_DISP}) · [PDF]({ASTHMA_DISP_PDF}) | [CACI Asthma Worksheet]({ASTHMA_CACI})")
        ...
        if all(x == "Yes" for x in [a1, a2, a3, a4, a5, a6, a7, a8]):
            show_result("CACI Issue", 
                ...
                reference=f"[CACI Asthma Worksheet]({ASTHMA_CACI}) | [Asthma Disposition Table]({ASTHMA_DISP}) · [PDF]({ASTHMA_DISP_PDF})")
...
if "18.g" in selected:
    ...
    if "Significant" in heart:
        show_result("Defer", "Most significant cardiac conditions require initial FAA review (Defer). Follow-up may use AASI protocols if previously authorized.",
                    reference=f"[AASI / Special Issuance list]({AASI_LIST}) | [Disposition Tables index]({DISP_INDEX})")
        ...
        show_result("Defer", "Even apparently minor cardiac history is usually deferred on first presentation for FAA review. When in doubt, defer.",
                    reference=f"[Disposition Tables index]({DISP_INDEX}) | [AASI list]({AASI_LIST})")
    ...
    if bp.startswith("A."):
        ...
        if bp_ok == "Yes":
            show_result("Issue",
                ...
                reference=f"[Hypertension Disposition Table]({HYPERTENSION_DISP}) | [Blood Pressure Item 55]({BP_DISP}) | [CACI Hypertension Worksheet]({HYPERTENSION_CACI})")
        ...
        show_result("Defer",
            ...
            reference=f"[Hypertension Disposition Table]({HYPERTENSION_DISP}) | [CACI Hypertension Worksheet]({HYPERTENSION_CACI})")
        ...
        if all(x == "Yes" for x in [h1, h2, h3, h4, h5]) and h6.startswith("Yes"):
            show_result("CACI Issue",
                ...
                reference=f"[CACI Hypertension Worksheet]({HYPERTENSION_CACI}) | [Hypertension Disposition Table]({HYPERTENSION_DISP})")
    ...
    if "Prediabetes" in dia:
        ...
        if pre.startswith("Yes"):
            show_result("CACI Issue", "If the official CACI Prediabetes worksheet criteria are fully satisfied, you may issue. Keep documents on file.",
                        ...
                        reference=f"[CACI Prediabetes Worksheet]({PREDIABETES_CACI}) | [All CACI Worksheets]({CACI_MAIN})")
    ...
    **Important:** Most significant psychiatric conditions require FAA review.  
    ...
    if ment.startswith("Mild, fully resolved"):
        ...
        show_result("Defer",
            ...
            reference=f"[Psychiatric Disposition Table (Item 47)]({PSYCH_DISP}) | [AME Guide – Antidepressant pathway]({AME_GUIDE})")
...
if "18.p" in selected:
    ...
    show_result("Defer", "Any history of suicide attempt requires full psychiatric evaluation and FAA review.",
                    reference=f"[Psychiatric Disposition Table (Item 47)]({PSYCH_DISP})")
